import { useAdminDeletePartnerApplicationMutation } from "./adminPartnerApplicationsApiSlice";
...
function TableRow({ application }) {
  const [adminDeletePartnerApplication] =
  ...
  const deleteOneApplication = async (student) => {
    ...
    if (confirmer) {
      try {
        await adminDeletePartnerApplication({ id: application._id }).unwrap();
